...
public class EnemyAI : MonoBehaviour
{
   ...
   [SerializeField] 
   private float attractionDistance = 6f;
   ...
   [SerializeField]
   private int wanderingWaitTimeMax = 5;
   
   [SerializeField]
   private int wanderingDistanceMin = 3;
   
   [SerializeField]
   private int wanderingDistanceMax = 6;
   ...
   private void Update()
   {
      ...
      if (Vector3.Distance(_target.position, transform.position) <= attractionDistance)
      {
         ...
         if (!_isAttacking)
         {
            if (Vector3.Distance(_target.position, transform.position) <= attackRadius)
            {
   ...
   private IEnumerator GetNewDestination()
   {
      ...
      yield return new WaitForSeconds(rnd.Next(wanderingWaitTimeMin, wanderingWaitTimeMax));

      var nextDestination = transform.position;
      nextDestination += rnd.Next(wanderingDistanceMin, wanderingDistanceMax) * new Vector3(rnd.Next(-1,1),0, rnd.Next(-1,1)).normalized;

      if (NavMesh.SamplePosition(nextDestination, out var hit, wanderingDistanceMax, NavMesh.AllAreas))
      {
